...
def get_shallow_unet_mutual(size=None, Params_loss=None):
    '''Get a shallow U-Net model. This is the optimal shallow U-Net after our test.
        In training, "Params_loss" specifies the loss function.
    ...
    ## decoder 1
    x1 = tf.keras.layers.Conv2D(16, (3, 3), activation=activation, kernel_initializer='he_normal', padding='same')(do3)
    x1 = tf.keras.layers.Dropout(0.2)(x1)

    x1 = tf.keras.layers.Conv2DTranspose(8, (2, 2), strides=(2, 2), padding='same')(x1)
    # ct2 = tf.keras.layers.concatenate([ct2, do2], axis=3) # The second skip connection

    x1 = tf.keras.layers.Conv2D(8, (3, 3), activation=activation, kernel_initializer='he_normal', padding='same')(x1)
    x1 = tf.keras.layers.Dropout(0.1)(x1)

    x1 = tf.keras.layers.Conv2DTranspose(4, (2, 2), strides=(2, 2), padding='same')(x1)
    x1 = tf.keras.layers.concatenate([x1, do1], axis=3) # The first skip connection

    x1 = tf.keras.layers.Conv2D(4, (3, 3), activation=activation, kernel_initializer='he_normal', padding='same')(x1)
    x1 = tf.keras.layers.Dropout(0.1)(x1)

    output1 = tf.keras.layers.Conv2D(1, (1, 1), activation='sigmoid')(x1)
    
    ## decoder 2 - 2 skip connections
    x2 = tf.keras.layers.Conv2D(16, (3, 3), activation=activation, kernel_initializer='he_normal', padding='same')(do3)
    x2 = tf.keras.layers.Dropout(0.2)(x2)

    x2 = tf.keras.layers.Conv2DTranspose(8, (2, 2), strides=(2, 2), padding='same')(x2)
    x2 = tf.keras.layers.concatenate([x2, do2], axis=3) # The second skip connection

    x2 = tf.keras.layers.Conv2D(8, (3, 3), activation=activation, kernel_initializer='he_normal', padding='same')(x2)
    x2 = tf.keras.layers.Dropout(0.1)(x2)

    x2 = tf.keras.layers.Conv2DTranspose(4, (2, 2), strides=(2, 2), padding='same')(x2)
    x2 = tf.keras.layers.concatenate([x2, do1], axis=3) # The first skip connection

    x2 = tf.keras.layers.Conv2D(4, (3, 3), activation=activation, kernel_initializer='he_normal', padding='same')(x2)
    x2 = tf.keras.layers.Dropout(0.1)(x2)

    output2 = tf.keras.layers.Conv2D(1, (1, 1), activation='sigmoid')(x2)

    ## decoder 3 - 16,16,16 
    x3 = tf.keras.layers.Conv2D(16, (3, 3), activation=activation, kernel_initializer='he_normal', padding='same')(do3)
    x3 = tf.keras.layers.Dropout(0.2)(x3)

    x3 = tf.keras.layers.Conv2DTranspose(16, (2, 2), strides=(2, 2), padding='same')(x3)
    #x2 = tf.keras.layers.concatenate([x2, do2], axis=3) # The second skip connection

    x3 = tf.keras.layers.Conv2D(16, (3, 3), activation=activation, kernel_initializer='he_normal', padding='same')(x3)
    x3 = tf.keras.layers.Dropout(0.1)(x3)

    x3 = tf.keras.layers.Conv2DTranspose(4, (2, 2), strides=(2, 2), padding='same')(x3)
    x3 = tf.keras.layers.concatenate([x3, do1], axis=3) # The first skip connection

    x3 = tf.keras.layers.Conv2D(4, (3, 3), activation=activation, kernel_initializer='he_normal', padding='same')(x3)
    x3 = tf.keras.layers.Dropout(0.1)(x3)

    output3 = tf.keras.layers.Conv2D(1, (1, 1), activation='sigmoid')(x3)
    
    
    #outputs = output1 #/2 + output2/2 + output3/3
    ...
    model1 = tf.keras.Model(inputs=[inputs], outputs=[output2])
    model1.compile(optimizer='adam', loss=loss_func, metrics=[dice_loss])
    model1.summary()
    
    model2 = tf.keras.Model(inputs=[inputs], outputs=[output3])
    model2.compile(optimizer='adam', loss=loss_func, metrics=[dice_loss])
    model2.summary()
    # model.compile(optimizer=tf.keras.optimizers.Adam(lr=1e-4), loss=loss_func, metrics=[dice_loss])
    return model,model1,model2

def get_shallow_unet_gan(discriminator, decoder, size=None, Params_loss=None):
    '''Get a shallow U-Net model. This is the optimal shallow U-Net after our test.
        In training, "Params_loss" specifies the loss function.
    ...
    c1 = discriminator.get_layer(index = 1)(inputs)
    do1 = discriminator.get_layer(index = 2)(c1)

    mp1 = discriminator.get_layer(index = 3)(do1)
    c2 = discriminator.get_layer(index = 4)(mp1)
    do2 = discriminator.get_layer(index = 5)(c2)

    mp2 =  discriminator.get_layer(index = 6)(do2)
    c3 =  discriminator.get_layer(index = 7)(mp2)
    do3 =  discriminator.get_layer(index = 8)(c3)

    c3 = decoder.get_layer(index = 3)(do3)
    do3 = decoder.get_layer(index = 4)(c3)
    
    ct2 = decoder.get_layer(index = 5)(do3)
    c2 = decoder.get_layer(index = 6)(ct2)
    do2 = decoder.get_layer(index = 7)(c2)
    
    ct1 = decoder.get_layer(index = 8)(do2)
    ct1 = tf.keras.layers.concatenate([ct1, do1], axis=3) # The first skip connection
    #c1 = decoder.get_layer(index = 9)(ct1)
    #do1 = decoder.get_layer(index = 10)(c1)
    # c3 = tf.keras.layers.Conv2D(16, (3, 3), activation=activation, kernel_initializer='he_normal', padding='same')(do3)
    # do3 = tf.keras.layers.Dropout(0.2)(c3)
...
def get_shallow_unet_2(size=None, Params_loss=None):
    '''Get a shallow U-Net model. This is the optimal shallow U-Net after our test.
        In training, "Params_loss" specifies the loss function.
...
def get_shallow_unet_3(size=None, Params_loss=None):
    '''Get a shallow U-Net model. This is the optimal shallow U-Net after our test.
        In training, "Params_loss" specifies the loss function.
    ...
    model = tf.keras.Model(inputs=[inputs], outputs=[output3])
    model.compile(optimizer='adam', loss=loss_func, metrics=[dice_loss])
    #model.summary()
